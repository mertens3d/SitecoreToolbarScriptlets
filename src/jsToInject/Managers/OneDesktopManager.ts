...
export class OneDesktopManager extends ManagerBase {
  ...
  GetAllLiveIframeData(targetWindow: IDataBrowserWindow): IDataOneIframe[] {
    ...
    if (iframeAr) {
      ...
      for (var ifrIdx = 0; ifrIdx < iframeAr.length; ifrIdx++) {
        ...
        var dataOneIframe: IDataOneIframe = this.Utilites().DateOneIframeFactory(iframeElem, targetWindow.DataDocSelf, 'desktop Iframe_' + ifrIdx);
