...
class ButtonVisibilityTests_ButtonVisibilityTester {
    ...
    VisibilityTestSnapShotSelected(currSelSnapshot) {
        let toReturn = false;
        if (currSelSnapshot && currSelSnapshot.AsBracedGuid() !== Guid_Guid.GetEmptyGuid().AsBracedGuid()) {
            toReturn = true;
        }
        return toReturn;
    }
...
class SelectSnapshotModule_SelectSnapshotModule {
    ...
    BuildOne(data, prior, idx) {
        ...
        if ((data.Id && prior && data.Id.ToString() === prior.ToString()) ||
            (idx === 0 && !prior)
            ||
                (idx === 0 && prior.ToString() === Guid_Guid.GetEmptyGuid().ToString())) {
...
class UiManager_UiManager extends PopUpManagerBase {
    ...
    FixGuidsInContentState(dataAr) {
        let toReturn = [];
        if (dataAr) {
            for (var idx = 0; idx < dataAr.length; idx++) {
                let candidate = dataAr[idx];
                if (Guid_Guid.IsValidGuidStr(candidate.Id.Raw)) {
