...
class StaticHelpers_StaticHelpers {
    ...
    static MakeUrlParts(url) {
        let toReturn = {
            FullUrl: url,
            Hostname: StaticHelpers_StaticHelpers.ExtractHostName(url),
            QueryString: ''
        };
        return toReturn;
    }
...
class UiManager_UiManager extends PopUpManagerBase {
    ...
    Init() {
        this.debug().FuncStart(UiManager_UiManager.name, this.Init.name);
        var self = this;
        ...
        this.debug().FuncEnd(UiManager_UiManager.name, this.Init.name);
    }
    WriteBuildNumToUi() {
    ...
    PopulateContentStateDiv(contentState) {
        var targetCurrStateDiv = window.document.querySelector(this.Const().Selector.HS.DivState);
        var allTaText = 'State as of: ' + this.Helpers().UtilityHelp.MakeFriendlyDate(new Date());
        if (targetCurrStateDiv) {
            allTaText += '\n';
            allTaText += 'Page Type: ' + StaticHelpers_StaticHelpers.WindowTypeAsString(this.TabMan().CurrentTabData.ScWindowType);
...
class ExternalEvents_ExternalEvents extends CommonEvents_CommonEvents {
    __hndlrAddCETab() {
        return ExternalEvents_awaiter(this, void 0, void 0, function* () {
            ...
            this.MsgMan().SendMessageToContentTab(new MsgPayloadRequestFromPopUp_MsgFromPopUp(MsgFlag.ReqAddCETab, this.PopHub), this.TabMan().CurrentTabData);
        });
    }
    ...
    HndlrSnapShotRestore(evt) {
        return ExternalEvents_awaiter(this, void 0, void 0, function* () {
            ...
            this.__initNewOperation();
            yield this.CreateNewWindowIfRequired(evt, this.TabMan().CurrentTabData.Tab.url)
                .then((newTab) => {
...
class EventManager_EventManager extends PopUpManagerBase {
    ...
    Init() {
        this.debug().FuncStart(EventManager_EventManager.name + this.Init.name);
        this.BuildAllCommands();
        this.__wireMenuButtons();
        this.debug().FuncEnd(EventManager_EventManager.name + this.Init.name);
    }
    BuildAllCommands() {
...
class PopUpMessagesManager_PopUpMessagesManager extends PopUpManagerBase {
    ...
    SendMessageToContentTab(msgPlayload, targetTab) {
        return PopUpMessagesManager_awaiter(this, void 0, void 0, function* () {
            this.debug().FuncStart(this.SendMessageToContentTab.name, StaticHelpers_StaticHelpers.MsgFlagAsString(msgPlayload.MsgFlag));
            ...
            this.debug().FuncEnd(this.SendMessageToContentTab.name, StaticHelpers_StaticHelpers.MsgFlagAsString(msgPlayload.MsgFlag));
        });
    }
...
class PopUpAtticManager_PopUpAtticManager extends PopUpManagerBase {
    Init() {
        this.debug().FuncStart(PopUpAtticManager_PopUpAtticManager.name, this.Init.name);
        this.DefaultSettings = this.GetDefaultSettings();
        this.debug().FuncEnd(PopUpAtticManager_PopUpAtticManager.name, this.Init.name);
    }
    __drawSettings() {
...
class TabManager_TabManager extends PopUpManagerBase {
    constructor(hub) {
        super(hub);
        hub.debug.FuncStart(TabManager_TabManager.name);
        hub.debug.FuncEnd(TabManager_TabManager.name);
    }
    MakeTabData(rawTab) {
        let toReturn = {
    ...
    Init() {
        return TabManager_awaiter(this, void 0, void 0, function* () {
            this.debug().FuncStart(TabManager_TabManager.name, this.Init.name);
            yield this.GetAssociatedTab()
                .then((tabData) => {
                this.CurrentTabData = tabData;
                this.CurrentTabData.UrlParts = StaticHelpers_StaticHelpers.MakeUrlParts(tabData.Tab.url);
            });
            this.debug().DebugIDataBrowserTab(this.CurrentTabData);
            this.debug().FuncEnd(TabManager_TabManager.name, this.Init.name);
        });
    }
    GetAssociatedTab() {
        return TabManager_awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => TabManager_awaiter(this, void 0, void 0, function* () {
                ...
                yield browser.tabs.query({ currentWindow: true, active: true })
                    ...
                    toReturn = {
                        ScWindowType: scWindowType.Unknown,
                        Tab: tabs[0],
                        UrlParts: StaticHelpers_StaticHelpers.MakeUrlParts(tabs[0].url)
                    };
    ...
    SetWindowDataToCurrent(tab, nickname) {
        var toReturn = {
            Tab: tab,
            ScWindowType: scWindowType.Unknown,
            UrlParts: {
...
class LocationManager_LocationManager extends PopUpManagerBase {
    ...
    SetScModeFromEditPrevNorm(newValue, currentPageType) {
        return LocationManager_awaiter(this, void 0, void 0, function* () {
            var currentTabHref = this.TabMan().CurrentTabData.Tab.url;
    ...
    SetScMode(newValue, useOrigWindow) {
        return new Promise(() => LocationManager_awaiter(this, void 0, void 0, function* () {
            ...
            var itemGuid;
            var targetWindow;
            var currentPageType = this.TabMan().CurrentTabData.ScWindowType;
            if (currentPageType === scWindowType.ContentEditor
                ...
                yield this.SetScModeFromEditPrevNorm(newValue, currentPageType);
            }
...
class PopUpHub_PopUpHub {
    ...
    init() {
        return PopUpHub_awaiter(this, void 0, void 0, function* () {
            this.debug.FuncStart(PopUpHub_PopUpHub.name, this.init.name);
            ...
            this.PopMsgMan.Init();
            this.UiMan.Init();
            this.debug.FuncEnd(PopUpHub_PopUpHub.name, this.init.name);
            this.debug.Log('');
            this.debug.Log('');
        });
    }
