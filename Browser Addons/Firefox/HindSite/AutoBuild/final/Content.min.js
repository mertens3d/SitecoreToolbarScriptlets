...
class ContentAtticManager_ContentAtticManager extends ContentManagerBase {
    constructor(hub, logger) {
        super(hub, logger);
        this._logger = logger;
        this._logger.FuncStart(ContentAtticManager_ContentAtticManager.name);
        this._logger.FuncEnd(ContentAtticManager_ContentAtticManager.name);
    }
    Init() {
    }
    UpdateNickname(payload) {
        return new Promise((resolve, reject) => {
            ...
            var promResult = new PromiseResult(this.UpdateNickname.name, this._logger);
    ...
    MarkFavorite(data) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            var result = new PromiseResult(this.MarkFavorite.name, this._logger);
            if (data.IdOfSelect) {
    ...
    WriteToStorage(dataOneWindow) {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                ...
                var result = new PromiseResult(this.WriteToStorage.name, this._logger);
            ...
            for (var idx = 0; idx < cacheLength; idx++) {
                ...
                if (candidate.Flavor) {
                    if (autoCount > InjectConst_ContentConst.Const.MaxAutoToSaveCount) {
                        this._logger.LogVal('Delete (max count :' + InjectConst_ContentConst.Const.MaxAutoToSaveCount + ')', candidate.TimeStamp.toString());
                        deleteFlag = true;
                    }
                    autoCount++;
                }
                if (now.getTime() - candidate.TimeStamp.getTime() > maxAutoSaveDiff) {
                    this._logger.LogVal('Delete (Old : max' + InjectConst_ContentConst.Const.MaxAutoSaveAgeDays + ' days)', candidate.TimeStamp.toString());
                    deleteFlag = true;
                }
                if (!deleteFlag) {
    ...
    GetAllSnapShotsMany(cacheMode) {
        this._logger.FuncStart(this.GetAllSnapShotsMany.name, StaticHelpers_StaticHelpers.CacheModeAsString(cacheMode));
        var toReturn;
        ...
        this._logger.LogVal('cached timeDiff', timeDiff + ' : ' + InjectConst_ContentConst.Const.MaxCacheAgeMs);
...
class MiscManager_MiscManager extends ContentManagerBase {
    constructor(hub, logger) {
        super(hub, logger);
        hub.Logger.FuncStart(MiscManager_MiscManager.name);
        hub.Logger.FuncEnd(MiscManager_MiscManager.name);
    }
    ...
    GetDialogIframe0Blue(dataPublishChain = null) {
        return PromiseChainQuickPublish_awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => PromiseChainQuickPublish_awaiter(this, void 0, void 0, function* () {
                ...
                let promiseResult = new PromiseResult(this.GetDialogIframe0Blue.name, this.Log());
...
class PromiseChainRestoreDesktop_PromiseChainRestoreDesktop extends ContentManagerBase {
    constructor(hub, logger) {
        super(hub, logger);
        hub.Logger.FuncStart(PromiseChainRestoreDesktop_PromiseChainRestoreDesktop.name);
        hub.Logger.FuncEnd(PromiseChainRestoreDesktop_PromiseChainRestoreDesktop.name);
    }
    ...
    __waitForIframeCountDiff(promiseBucket) {
        return new Promise((resolve, reject) => PromiseChainRestoreDesktop_awaiter(this, void 0, void 0, function* () {
            ...
            var result = new PromiseResult(this.__waitForIframeCountDiff.name, this.Log());
...
class OneTreeManager_OneTreeManager extends ContentManagerBase {
    constructor(hub, associatedDoc, logger) {
        super(hub, logger);
        hub.Logger.FuncStart(OneTreeManager_OneTreeManager.name);
        this.AssociatedDoc = associatedDoc;
        hub.Logger.FuncEnd(OneTreeManager_OneTreeManager.name);
    }
    ...
    GetOneLiveTreeData() {
        ...
        if (this.AssociatedDoc) {
            this.Log().LogVal('Looking for node ID: ', InjectConst_ContentConst.Const.ElemId.sc.SitecoreRootNodeId);
...
class OneCEManager_OneCEManager extends ContentManagerBase {
    constructor(hub, associatedDoc, logger) {
        super(hub, logger);
        this.AssociatedDoc = associatedDoc;
        this.OneTreeMan = new OneTreeManager_OneTreeManager(hub, this.AssociatedDoc, logger);
    }
    ...
    __collapseRootNode(targetCEDoc) {
        ...
        if (rootElem) {
            ...
            this.Log().Error(this.__collapseRootNode.name, 'Root glyph not found ' + InjectConst_ContentConst.Const.ElemId.sc.SitecoreRootGlyphId);
        }
    }
    ...
    GetStateCe(id) {
        return new Promise((resolve, reject) => {
            ...
            let result = new PromiseResult(this.GetStateCe.name, this.Log());
            var toReturnCEData = {
                Id: id,
...
class OneDesktopManager_OneDesktopManager extends ContentManagerBase {
    constructor(hub, associatedDoc, logger) {
        super(hub, logger);
        hub.Logger.FuncStart(OneDesktopManager_OneDesktopManager.name);
        this.associatedDoc = associatedDoc;
        hub.Logger.FuncEnd(OneDesktopManager_OneDesktopManager.name);
    }
        ...
        return OneDesktopManager_awaiter(this, void 0, void 0, function* () {
            ...
            if (this.MiscMan().NotNullOrUndefined([targetDoc, dataToRestore, dataToRestore.AllCEAr], this.RestoreDesktopState.name)) {
                for (var idx = 0; idx < dataToRestore.AllCEAr.length; idx++) {
                    ...
                    var desktopPromiser = new PromiseChainRestoreDesktop_PromiseChainRestoreDesktop(this.ContentHub, this.ContentLogger);
    ...
    GetAllLiveIframeData() {
        ...
        this.Log().DebugIDataOneDoc(this.associatedDoc);
        var toReturn = [];
        var iframeAr = this.associatedDoc.ContentDoc.querySelectorAll(InjectConst_ContentConst.Const.Selector.SC.IframeContent);
    ...
    GetStateDesktop() {
        return new Promise((resolve, reject) => OneDesktopManager_awaiter(this, void 0, void 0, function* () {
            ...
            this.Log().DebugIDataOneDoc(this.associatedDoc);
            let promiseResult = new PromiseResult(this.GetStateDesktop.name, this.Log());
            ...
            if (toReturnAllCeState.livingIframeAr && toReturnAllCeState.livingIframeAr.length > 0) {
                for (var iframeIdx = 0; iframeIdx < toReturnAllCeState.livingIframeAr.length; iframeIdx++) {
                    ...
                    var oneCeMan = new OneCEManager_OneCEManager(this.ContentHub, targetIframeObj.ContentDoc, this.ContentLogger);
...
class OneScWindowManager_OneScWindowManager extends ContentManagerBase {
    constructor(hub, logger) {
        ...
        this.OneCEMan = null;
        hub.Logger.FuncStart(OneScWindowManager_OneScWindowManager.name);
        hub.Logger.FuncEnd(OneScWindowManager_OneScWindowManager.name);
    }
    Init() {
        let currPageType = this.ScUiMan().GetCurrentPageType();
        if (currPageType === scWindowType.Desktop) {
            this.OneDesktopMan = new OneDesktopManager_OneDesktopManager(this.ContentHub, this.ScUiMan().TopLevelDoc(), this.ContentLogger);
        }
        else if (currPageType === scWindowType.ContentEditor) {
            this.OneCEMan = new OneCEManager_OneCEManager(this.ContentHub, this.ScUiMan().TopLevelDoc(), this.ContentLogger);
        }
    }
    SaveWindowState(snapShotSettings) {
        return new Promise((resolve, reject) => OneScWindowManager_awaiter(this, void 0, void 0, function* () {
            ...
            let promiseResult = new PromiseResult(this.SaveWindowState.name, this.Log());
    ...
    PublishActiveCE(targetDoc) {
        return OneScWindowManager_awaiter(this, void 0, void 0, function* () {
            ...
            if (docToPublish) {
                var publishChain = new PromiseChainQuickPublish_PromiseChainQuickPublish(this.ContentHub, this.ContentLogger);
...
class PromiseOneStep_PromiseOneStep extends ContentManagerBase {
    constructor(hub, logger) {
        super(hub, logger);
        hub.Logger.FuncStart(PromiseGeneric_PromiseHelper.name);
        hub.Logger.FuncEnd(PromiseGeneric_PromiseHelper.name);
    }
...
class ContentMessageManager_ContentMessageManager extends ContentManagerBase {
    constructor(contentHub, logger) {
        ...
        contentHub.Logger.FuncStart(ContentMessageManager_ContentMessageManager.name);
        var self = this;
        browser.runtime.onMessage.addListener(request => self.ContentReceiveRequest(request));
        contentHub.Logger.FuncEnd(ContentMessageManager_ContentMessageManager.name);
    }
    ...
    ScheduleIntervalTasks(reqMsgFromPopup) {
        ...
        let autoSaveSetting = this.Helpers().SettingsHelp.GetByKey(SettingKey.AutoSaveIntervalMin, reqMsgFromPopup.CurrentContentPrefs);
        if (SettingsHelper_SettingsHelper.ValueAsInteger(autoSaveSetting) > 0) {
    ...
    SetLoggerFromMessage(reqMsgFromPopup) {
        let currSetting = this.Helpers().SettingsHelp.GetByKey(SettingKey.LogToConsole, reqMsgFromPopup.CurrentContentPrefs);
        let valueToUse = SharedConst.Const.Settings.Defaults.LogToConsole;
        if (currSetting) {
            let candidate = SettingsHelper_SettingsHelper.ValueAsBool(currSetting);
            if (candidate) {
    ...
    ContentReceiveRequest(reqMsgFromPopup) {
        return ContentMessageManager_awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => ContentMessageManager_awaiter(this, void 0, void 0, function* () {
                this.Log().FuncStart(this.ContentReceiveRequest.name, StaticHelpers_StaticHelpers.MsgFlagAsString(reqMsgFromPopup.MsgFlag));
                var promResult = new PromiseResult(this.ContentReceiveRequest.name, this.Log());
                this.Log().LogAsJsonPretty(MsgFromPopUp_MsgFromPopUp.name, reqMsgFromPopup);
                var response = null;
                if (reqMsgFromPopup) {
                ...
                if (response !== null) {
                    this.Log().LogVal('responding', StaticHelpers_StaticHelpers.MsgFlagAsString(response.MsgFlag));
                }
                this.Log().FuncEnd(this.ContentReceiveRequest.name, StaticHelpers_StaticHelpers.MsgFlagAsString(reqMsgFromPopup.MsgFlag));
                if (promResult.WasSuccessful()) {
                    resolve(response);
                }
                else {
    ...
    Init() {
        this.Log().FuncStart(this.Init.name + ' ' + ContentMessageManager_ContentMessageManager.name);
        var self = this;
    ...
    ReqMsgRouter(payload) {
        return ContentMessageManager_awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => ContentMessageManager_awaiter(this, void 0, void 0, function* () {
                this.Log().FuncStart(this.ReqMsgRouter.name, StaticHelpers_StaticHelpers.MsgFlagAsString(payload.MsgFlag));
                this.Log().LogAsJsonPretty(MsgFromPopUp_MsgFromPopUp.name, payload);
                let promiseResult = new PromiseResult(this.ReqMsgRouter.name, this.Log());
                ...
                switch (payload.MsgFlag) {
                    ...
                    case MsgFlag.ReqAdminB:
                        ...
                        break;
                    case MsgFlag.Ping:
                        this.Log().LogVal('Ping', StaticHelpers_StaticHelpers.MsgFlagAsString(payload.MsgFlag));
                    ...
                    case MsgFlag.ReqUpdateNickName:
                        ...
                        break;
                    default:
                        this.Log().Error('Unhandled MsgFlag', StaticHelpers_StaticHelpers.MsgFlagAsString(payload.MsgFlag));
    ...
    __restoreClick(Data) {
        return new Promise((resolve, reject) => ContentMessageManager_awaiter(this, void 0, void 0, function* () {
            let promiseResult = new PromiseResult(this.__restoreClick.name, this.Log());
            try {
                this.Log().MarkerA();
...
class ContentFactories_ContentFactories extends ContentManagerBase {
    ...
    UpdateContentState(contentState) {
        return new Promise((resolve, reject) => ContentFactories_awaiter(this, void 0, void 0, function* () {
            ...
            let promiseResult = new PromiseResult(this.UpdateContentState.name, this.ContentLogger);
    ...
    GetCurrentDtOrCeState() {
        return new Promise((resolve, reject) => ContentFactories_awaiter(this, void 0, void 0, function* () {
            ...
            let promiseResult = new PromiseResult(this.GetCurrentDtOrCeState.name, this.ContentLogger);
...
class ContentHub_ContentHub {
    constructor(logger) {
        ...
        logger.FuncStart(ContentHub_ContentHub.name);
        this.Logger = logger;
        console.log('(ctor) logger enabled ' + this.Logger.EnabledStatus());
        this.Instantiate();
        logger.FuncEnd(ContentHub_ContentHub.name);
    }
    Instantiate() {
        this.Logger.FuncStart(this.Instantiate.name);
        this.AtticMan = new ContentAtticManager_ContentAtticManager(this, this.Logger);
        this.Helpers = new Helpers_HelperHub(this.Logger);
        this.MsgMan = new ContentMessageManager_ContentMessageManager(this, this.Logger);
        this.MiscMan = new MiscManager_MiscManager(this, this.Logger);
        this.ContentFactory = new ContentFactories_ContentFactories(this, this.Logger);
        this.Logger.MarkerA();
        this.OneWindowMan = new OneScWindowManager_OneScWindowManager(this, this.Logger);
        this.PromiseOneStep = new PromiseOneStep_PromiseOneStep(this, this.Logger);
        this.SharedConst = SharedConst.Const;
        this.SitecoreUiMan = new SitecoreUiManager_SitecoreUiManager(this, this.Logger);
        this.Init();
    ...
    Init() {
        this.Logger.FuncStart(ContentHub_ContentHub.constructor.name + ' ' + this.Init.name);
        this.Const = InjectConst_ContentConst.Const;
        this.AtticMan.Init();
        this.MsgMan.Init();
        this.Logger.SetEnabled(this.MsgMan.IsLogEnabled());
        this.OneWindowMan.Init();
        this.Logger.FuncEnd(ContentHub_ContentHub.constructor.name + ' ' + this.Init.name);
    }
}
...
// CONCATENATED MODULE: ./AutoBuild/TsTranspiled/Content/scripts/Agents/LoggerContent.js


class LoggerContent_LoggerContentAgent extends LoggerAgentBase_LoggerAgentBase {
...
new ContentHub_ContentHub(new LoggerContent_LoggerContentAgent());
