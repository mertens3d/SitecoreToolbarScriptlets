...
class ContentAtticManager_ContentAtticManager extends ContentManagerBase {
    ...
    GetFromStorageById(needleId, noCache = false) {
        this.debug().FuncStart(this.GetFromStorageById.name, needleId.AsString);
        var foundStorage = this.GetAllSnapShotsMany(noCache);
    ...
    GetAllSnapShotsMany(noCache = false) {
    ...
    RemoveOneFromStorage(targetId) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (targetId) {
                    var storageMatch = yield this.GetFromStorageById(targetId, true);
                    if (storageMatch) {
                        var result = confirm('Remove ?: ' + this.Xyyz.Utilities.TimeNicknameFavStrForConfirmation(storageMatch));
                        if (result === true) {
                            this.debug().LogVal('Key to Delete', storageMatch.RawData.key);
                            yield window.localStorage.removeItem(storageMatch.RawData.key);
                            var stillExists = yield this.GetFromStorageById(targetId, true);
                            if (stillExists) {
                                successful = false;
...
class Utilities_Utilities {
    ...
    SelectHeaderStr(prefix) {
        let toReturn = '';
        toReturn += StaticHelpers_StaticHelpers.BufferString(prefix, 6, ' ', false);
        toReturn += StaticHelpers_StaticHelpers.BufferString('...Time Stamp', 23, '.', false);
        toReturn += StaticHelpers_StaticHelpers.BufferString('- Page Type', 20, '.', false, false);
        toReturn += StaticHelpers_StaticHelpers.BufferString('- Nickname', 16, '.', false, false);
        toReturn += StaticHelpers_StaticHelpers.BufferString('- Fav.', 4, '.', false);
        toReturn += StaticHelpers_StaticHelpers.BufferString('- Node.', 10, '.', false);
        return toReturn;
    }
    ...
    TimeNicknameFavStr(data) {
        ...
        return this.MakeFriendlyDate(data.TimeStamp)
            + ' - ' + StaticHelpers_StaticHelpers.BufferString(typeStr, 17, ' ', false)
            + ' - ' + StaticHelpers_StaticHelpers.BufferString(data.NickName, 16, ' ', false)
            + ' - ' + StaticHelpers_StaticHelpers.BufferString((data.Flavor == SnapShotFlavor.Favorite ? '*' : ' '), 1)
            + ' - ' + StaticHelpers_StaticHelpers.BufferString((data.Flavor == SnapShotFlavor.Autosave ? 'A' : ' '), 1);
    }
    ...
    MakeFriendlyDate(date) {
        ...
        var month = StaticHelpers_StaticHelpers.BufferString(date.getMonth().toString(), 2, '0');
        var day = StaticHelpers_StaticHelpers.BufferString(date.getDay().toString(), 2, '0');
        var min = StaticHelpers_StaticHelpers.BufferString(date.getMinutes().toString(), 2, '0');
        var hoursRaw = date.getHours();
        var ampm = hoursRaw >= 12 ? 'pm' : 'am';
        hoursRaw = hoursRaw % 12;
        var hourClean = hoursRaw ? hoursRaw : 12;
        var hourCleanStr = StaticHelpers_StaticHelpers.BufferString(hourClean.toString(), 2, '0');
...
class ContentMessageManager_ContentMessageManager extends ContentManagerBase {
    ...
    respondFail() {
        this.SendMessageHndlr(new MsgPayloadResponseFromContent_MsgFromContent(MsgFlag.RespTaskFailed));
    }
...
class Factories_Factories extends ContentManagerBase {
    ...
    UpdateContentState(response) {
        return Factories_awaiter(this, void 0, void 0, function* () {
            response.State.SnapShotsMany = yield this.AtticMan().GetAllSnapShotsMany();
            response.State.WindowType = yield this.PageDataMan().GetCurrentPageType();
...
class debug_BaseDebug {
    ...
    LogVal(textValName, textVal) {
        ...
        textValName = StaticHelpers_StaticHelpers.BufferString(textValName.toString(), 26, ' ', false, false);
        const debugPrefix = '  ~~~  ';
