...
// CONCATENATED MODULE: ./AutoBuild/TsTranspiled/Shared/scripts/Classes/Defaults/DefaultStateOfTreeNode.js


class DefaultStateOfTreeNode_DefaultStateOfTreeNode {
    constructor() {
        this.IsActive = false;
        this.IsExpanded = false;
        this.NodeFriendly = '';
        this.NodeId = new GuidData_GuidData();
        this.Discriminator = SharedConst.Const.ObjDiscriminator.DataOneTreeNode;
    }
}

// CONCATENATED MODULE: ./AutoBuild/TsTranspiled/Shared/scripts/Classes/Defaults/DefaultStateOfTree.js

class DefaultStateOfTree_DefaultStateOfTree {
    constructor() {
        this.ActiveNode = new DefaultStateOfTreeNode_DefaultStateOfTreeNode();
    }
}
...
class ContentEditorTreeNodeProxy_TreeNodeProxy extends LoggableBase {
    constructor(logger, AssociatedDoc, sourceElement) {
        super(logger);
        this.Logger.InstantiateStart(ContentEditorTreeNodeProxy_TreeNodeProxy.name);
        if (sourceElement && AssociatedDoc) {
            ...
            if (sourceElement.hasAttribute('src')) {
                ...
                this.Logger.ErrorAndThrow(ContentEditorTreeNodeProxy_TreeNodeProxy.name, 'invalid source element type: ' + (typeof sourceElement));
            }
        }
        else {
            this.Logger.ErrorAndThrow(ContentEditorTreeNodeProxy_TreeNodeProxy.name, 'null sourceElement or associatedDoc');
        }
        this.Logger.InstantiateEnd(ContentEditorTreeNodeProxy_TreeNodeProxy.name);
    }
    ...
    GetParentTreeNode() {
        ...
        if (candidate) {
            ...
            toReturn = new ContentEditorTreeNodeProxy_TreeNodeProxy(this.Logger, this.OwnerDoc, candidate);
        }
        else {
    ...
    GetMainIconSrc() {
        ...
        if (penultimateElem) {
            penultimateNode = new ContentEditorTreeNodeProxy_TreeNodeProxy(this.Logger, this.OwnerDoc, penultimateElem);
        }
    ...
    SetStateOfTreeNode(newData) {
        ...
        if (newData.IsActive) {
            var hotTreeNodeId = ContentConst.Const.Names.SC.TreeGlyphPrefix + Guid_Guid.WithoutDashes(newData.NodeId);
            let hotTreeNode = this.OwnerDoc.ContentDoc.getElementById(hotTreeNodeId);
            if (hotTreeNode) {
                let hotTreeNodeProxy = new ContentEditorTreeNodeProxy_TreeNodeProxy(this.Logger, this.OwnerDoc, hotTreeNode);
                if (hotTreeNodeProxy) {
...
class Subject_ContentEditorTreeMutatedEvent_Subject_ContentEditorTreeMutatedEvent extends GenericEvent_Subject_GenericEvent_Subject {
    ...
    GetMutatedNode(mutation) {
        ...
        if (mutation.attributeName === 'class') {
            ...
            if (mutatedAnchorElement) {
                ...
                candidateNode = new ContentEditorTreeNodeProxy_TreeNodeProxy(this.Logger, this.HostDoc, mutatedAnchorElement);
...
class ContentEditorTreeProxy_TreeMutation_Observer extends LoggableBase {
    Update(payload) {
    }
}
class ContentEditorTreeProxy_TreeProxy extends LoggableBase {
    ...
    InitCeTreeProxy() {
        ...
        if (setting && setting.ValueAsBool()) {
            this.TreeMutationEvent = new Subject_ContentEditorTreeMutatedEvent_Subject_ContentEditorTreeMutatedEvent(this.Logger, this.GetTreeHolderElem(), this.HostIframeId, this.AssociatedDoc);
        }
    ...
    GetTreeNodeByGlyph(targetNode) {
        ...
        if (targetNode && this.AssociatedDoc) {
            ...
            this.Logger.Log('looking for: (' + targetNode.NodeFriendly + ')' + treeGlyphTargetId + ' in ' + Guid_Guid.AsShort(this.AssociatedDoc.DocId));
            var foundOnPageTreeGlyph = this.AssociatedDoc.ContentDoc.getElementById(treeGlyphTargetId);
            var test = this.AssociatedDoc.ContentDoc.querySelector('[id=' + treeGlyphTargetId + ']');
            if (foundOnPageTreeGlyph) {
                toReturn = new ContentEditorTreeNodeProxy_TreeNodeProxy(this.Logger, this.AssociatedDoc, foundOnPageTreeGlyph);
    ...
    SetStateOfTree(stateOfContentEditor) {
        return ContentEditorTreeProxy_awaiter(this, void 0, void 0, function* () {
            ...
            while (stateOfContentEditor.AllTreeNodeAr.length > 0 && iterHelper.DecrementAndKeepGoing()) {
                var nextNode = stateOfContentEditor.AllTreeNodeAr.shift();
    ...
    WalkNodeRecursive(targetNode, depth) {
        ...
        if (targetNode) {
            ...
            if (firstChildGlyphNode) {
                let treeNodeProxy = new ContentEditorTreeNodeProxy_TreeNodeProxy(this.Logger, this.AssociatedDoc, firstChildGlyphNode);
    ...
    GetRootNode() {
        ...
        let toReturn = this.AssociatedDoc.ContentDoc.getElementById(ContentConst.Const.ElemId.sc.SitecoreRootAnchorNodeId);
        return toReturn;
    }
    GetStateOfTree() {
        this.Logger.FuncStart(this.GetStateOfTree.name);
        let toReturnOneTreeState = new DefaultStateOfTree_DefaultStateOfTree();
        try {
...
class ContentEditorProxy_ContentEditorProxy extends LoggableBase {
    constructor(associatedDoc, logger, settingsAgent, parentIframeId) {
        ...
        this.AssociatedTreeProxy = new ContentEditorTreeProxy_TreeProxy(this.Logger, this.AssociatedDoc, this.SettingsAgent, this.ParentIframeId);
        this.Logger.InstantiateEnd(ContentEditorProxy_ContentEditorProxy.name);
    }
    GetStateOfContentEditor() {
        let toReturnStateOfContentEditor = {
            StateOfTree: null,
            Id: null,
        };
    ...
    SetStateTree(oneTreeState) {
        return ContentEditorProxy_awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => ContentEditorProxy_awaiter(this, void 0, void 0, function* () {
                ...
                if (oneTreeState) {
                    yield this.SetStateOfContentEditor(oneTreeState)
                        .then(() => resolve())
                        .catch((err) => {
...
class FrameProxy_FrameProxy extends LoggableBase {
        ...
        return FrameProxy_awaiter(this, void 0, void 0, function* () {
            try {
                ...
                yield recipeBasic.WaitForPageReadyHtmlIframeElement(this.IframeElem)
                    .then(() => this.ConEditProxy = new ContentEditorProxy_ContentEditorProxy(this.GetContentDoc(), this.Logger, this.SettingsAgent, this.IframeElem.id));
            }
            catch (err) {
    ...
    SetStateFrame(oneTreeState) {
        return FrameProxy_awaiter(this, void 0, void 0, function* () {
            yield this.ConEditProxy.SetStateTree(oneTreeState);
        });
    }
    GetStateOfFrame() {
        let toReturn = {
    ...
    GetState() {
        let oneCeState = this.ConEditProxy.GetStateOfTree();
        return oneCeState;
    }
    GetZindex() {
        let toReturn = -99;
...
class RecipeBasics_RecipeBasics extends LoggableBase {
    ...
    GetTopLevelIframe(targetDoc) {
        return RecipeBasics_awaiter(this, void 0, void 0, function* () {
            ...
            var toReturn = null;
            yield this.FrameHelper.GetHostedframes(targetDoc)
                .then((allIframe) => {
                var maxZVal = -1;
...
class RecipeSaveState_RecipeSaveState extends _RecipeBase_RecipeBase {
    ...
    Execute() {
        return new Promise((resolve, reject) => RecipeSaveState_awaiter(this, void 0, void 0, function* () {
            yield this.ScWinMan.GetStateOfSitecore(this.TargetSnapShotFlavor)
                .then((windowState) => {
...
class ContentAtticAgent_ContentAtticAgent {
    ...
    WriteStateOfSitecoreToStorage(dataOneWindow) {
    ...
    GetFromStorageById(needleId) {
        ...
        let foundStorage = this.GetStateOfSnapShots();
    ...
    CleanOutOldAutoSavedData() {
        ...
        try {
            ...
            let currentWindowStorage = this.GetStateOfSnapShots();
    ...
    GetStateOfSnapShots() {
        ...
        let snapShotsMany = new DefaultStateOfSnapshots();
    ...
    ConvertGuidData(candidateSnapShots) {
        ...
        for (var idx = 0; idx < candidateSnapShots.length; idx++) {
            ...
            try {
                if (candidate.GuidId && Guid_Guid.IsValidGuidStr(candidate.GuidId.Raw)) {
...
class DefaultStateOfFrame_DefaultStateOfFrame {
    constructor() {
        this.ContentEditorState = new DefaultStateOfContentEditor();
    }
}
...
class DefaultStateOfDesktop_DefaultStateOfDesktop {
    constructor() {
        this.StateOfActiveFrame = new DefaultStateOfFrame_DefaultStateOfFrame();
    }
}
...
class DefaultStateOfSitecoreWindow_DefaultStateOfSitecoreWindow {
    constructor() {
        this.StateOfSnapShots = new DefaultStateOfSnapshots();
        this.dateToUse = new Date();
        this.StateOfContentEditor = null;
        this.StateOfDesktop = new DefaultStateOfDesktop_DefaultStateOfDesktop();
        this.TimeStamp = this.dateToUse;
        this.TimeStampFriendly = StaticHelpers_StaticHelpers.MakeFriendlyDate(this.dateToUse);
        this.WindowType = ScWindowType.Unknown;
        this.WindowFriendly = ScWindowType[ScWindowType.Unknown];
        this.GuidId = Guid_Guid.NewRandomGuid();
        this.NickName = '';
        this.RawData = null;
...
class RecipeAddContentEditorToDesktop_RecipeAddNewContentEditorToDesktop extends LoggableBase {
    ...
    Execute() {
        return new Promise((resolve, reject) => RecipeAddContentEditorToDesktop_awaiter(this, void 0, void 0, function* () {
            ...
            let recipeBasics = new RecipeBasics_RecipeBasics(this.Logger, this.SettingsAgent);
            yield frameHelper.GetHostedframes(this.TargetDoc)
...
class RecipeToggleFavorite_RecipeToggleFavorite extends _RecipeBase_RecipeBase {
    ...
    Execute() {
        return new Promise((resolve, reject) => RecipeToggleFavorite_awaiter(this, void 0, void 0, function* () {
            if (this.TargetSnapShotId) {
                let result = this.AtticAgent.GetFromStorageById(this.TargetSnapShotId);
...
class ContentAPIManager_ContentAPIManager extends LoggableBase {
    constructor(logger, toastAgent, scUiMan, scWinMan, settingsAgent) {
        super(logger);
...
class DesktopStartBarProxy_TreeNodeChangedEvent_Observer extends LoggableBase {
    ...
    CallBackConEdProxyAdded(payload) {
        ...
        if (payload) {
            if (this.CeProxies.indexOf(payload.NewCeProxy) < 0) {
                ...
                let self = this;
                payload.NewCeProxy.RegisterObserver(new DesktopStartBarProxy_TreeNodeChangedEvent_Observer(this.Logger, this));
            }
        }
        else {
...
class DesktopIframeProxyBucket_DesktopIframeProxyBucket extends LoggableBase {
    ...
    InitHostedIframes() {
        return DesktopIframeProxyBucket_awaiter(this, void 0, void 0, function* () {
            try {
                yield this.GetFrameHelper().GetHostedframes(this.AssociatedDesktopDoc)
                    .then((foundIframes) => {
...
class DesktopProxy_DesktopDomChangedEvent_Observer extends LoggableBase {
    constructor(logger, owner) {
        super(logger);
        this.Owner = owner;
    }
...
class DesktopProxy_FrameMutation_Observer extends LoggableBase {
    constructor(logger, owner) {
        super(logger);
        this.Owner = owner;
    }
...
class DesktopProxy_DesktopProxy extends LoggableBase {
    constructor(logger, miscAgent, associatedDoc, settingsAgent) {
        ...
        let self = this;
        this.DesktopIframeProxyBucket.DesktopIframeProxyAddedEvent_Subject.RegisterObserver(new DesktopProxy_FrameMutation_Observer(self.Logger, self.DesktopStartBarAgent));
        this.Subject_DomChangedEvent = new Subject_DesktopDomChangedEvent_DesktopDomChangedEvent_Subject(this.Logger, this.AssociatedDoc);
        this.Subject_DomChangedEvent.RegisterObserver(new DesktopProxy_DesktopDomChangedEvent_Observer(this.Logger, this));
        this.Logger.InstantiateEnd(DesktopProxy_DesktopProxy.name);
    }
    InitDesktopProxy() {
        return DesktopProxy_awaiter(this, void 0, void 0, function* () {
            try {
                yield this.DesktopIframeProxyBucket.InitHostedIframes()
        ...
        return DesktopProxy_awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => DesktopProxy_awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    yield this.GetIframeHelper().GetHostedframes(this.AssociatedDoc)
                        .then((results) => {
                        if (results) {
            ...
            return new Promise((resolve, reject) => ScWindowRecipePartials_awaiter(this, void 0, void 0, function* () {
                ...
                if (dataToRestore) {
                    if (dataToRestore.WindowType == ScWindowType.Desktop) {
                        yield desktopProxy.SetStateOfDesktop(dataToRestore.StateOfDesktop)
                        ...
                        yield OneCEAgent.SetStateOfContentEditor(dataToRestore.StateOfContentEditor)
                            .then(() => this.ToastAgent.PopUpToastNotification(targetDoc, 'Restore Completed'))
                            .then(() => resolve())
                            .catch((err) => reject(err));
                    }
...
class ScWindowManager_ScWindowManager extends LoggableBase {
    ...
    GetStateOfSiteCoreWindow() {
        return new Promise((resolve, reject) => ScWindowManager_awaiter(this, void 0, void 0, function* () {
            ...
            yield this.DesktopProxy().GetStateOfDesktop()
                .then((result) => toReturnStateOfSitecoreWindow.StateOfDesktop = result)
                .then(() => this.ContentEditorProxy().GetStateOfContentEditor())
                .then((result) => toReturnStateOfSitecoreWindow.StateOfContentEditor = result)
    ...
    GetStateOfSitecore(snapShotFlavor) {
        return new Promise((resolve, reject) => ScWindowManager_awaiter(this, void 0, void 0, function* () {
            this.Logger.FuncStart(this.GetStateOfSitecore.name);
            let currentPageType = this.GetCurrentPageType();
            var toReturnStateOfSitecore = this.CreateShellIDataScWindowState(currentPageType, snapShotFlavor);
            yield this.DesktopProxy().GetStateOfDesktop()
                .then((stateOfDesktop) => toReturnStateOfSitecore.StateOfDesktop = stateOfDesktop)
                .then(() => this.ContentEditorProxy().GetStateOfContentEditor())
                .then((stateOfContentEditor) => toReturnStateOfSitecore.StateOfContentEditor = stateOfContentEditor)
...
class zContentEntryPoint_ContentEntry {
    ...
    InstantiateAndInitManagers() {
        return zContentEntryPoint_awaiter(this, void 0, void 0, function* () {
            ...
            this.ContentAPIMan = new ContentAPIManager_ContentAPIManager(this.Logger, this.ToastAgent, scUiMan, scWinMan, this.SettingsAgent);
