...
class HandlersExternal_HandlersExternal extends GenericEvent_Subject_GenericEvent_Subject {
    ...
    SendContentCommand(msgPlayload) {
        return new Promise((resolve, reject) => HandlersExternal_awaiter(this, void 0, void 0, function* () {
            ...
            yield this.MessageManager.SendMessageToContent(msgPlayload)
                .then((contentReplyPayload) => this.NotifyObservers(contentReplyPayload))
                .then(() => resolve())
                .catch((err) => reject(err));
...
class MessageManager_PopUpMessageManager extends LoggableBase {
...
class ScWindowStateValidator_ScWindowStateValidator {
    ...
    ValidateStateOfContentEditor(StateOfContentEditor) {
        ...
        if (!StateOfContentEditor.StateOfTree) {
            StateOfContentEditor.StateOfTree = new DefaultStateOfTree_DefaultStateOfTree();
        }
    ...
    ValidateStateOfDesktop(StateOfDesktop) {
        ...
        if (!StateOfDesktop.StateOfActiveFrame) {
            StateOfDesktop.StateOfActiveFrame = new DefaultStateOfFrame_DefaultStateOfFrame();
        }
...
class ButtonVisibilityTests_ButtonVisibilityTester extends LoggableBase {
    ...
    VisibilityTestActiveCeNode(stateOfSitecoreWindow) {
        let toReturn = false;
        if (stateOfSitecoreWindow) {
            let foundActive = stateOfSitecoreWindow.StateOfDesktop.StateOfActiveFrame.ContentEditorState;
            if (!foundActive) {
...
class UiButtonStateManager_UiButtonStateManager extends LoggableBase {
    constructor(logger) {
        super(logger);
        this.Logger.InstantiateStart(UiButtonStateManager_UiButtonStateManager.name);
        this.Logger.InstantiateEnd(UiButtonStateManager_UiButtonStateManager.name);
    }
    ...
    RefreshUiButtonState(currentWindowType, currSelSnapshot, stateOfSitecoreWindow, allMenuCommands) {
...
class SelectSnapshotModule_SelectSnapshotModule extends GenericEvent_Subject_GenericEvent_Subject {
    constructor(selector, logger) {
        super(logger);
    ...
    PopulateStateOfSnapShotSelect() {
        ...
        if (this.StateOfSnapShots.CurrentSnapShots) {
            let snapShots = this.StateOfSnapShots.CurrentSnapShots;
            if (snapShots) {
    ...
    GetFirstDataWithActiveNode(data) {
        ...
        if ((data.WindowType === ScWindowType.Desktop) && data.StateOfDesktop && data.StateOfDesktop.StateOfActiveFrame && data.StateOfDesktop.StateOfActiveFrame.ContentEditorState.StateOfTree) {
            ...
            this.Logger.WarningAndContinue(this.GetFirstDataWithActiveNode.name, 'Not implemented ' + StaticHelpers_StaticHelpers.ScWindowTypeFriendly(data.WindowType));
        }
        return toReturn;
    }
...
class FeedbackModuleBrowserState_FeedbackModuleBrowserState extends UiFeedbackModuleBase {
    ...
    PopulateFeedackBrowserState(scUrlAgent) {
...
class FeedbackModuleContentState_FeedbackModuleContentState extends UiFeedbackModuleBase {
    ...
    PopulateContentStateFeedack(stateOfSitecoreWindow) {
        var allStateText = '';
...
class UiManager_UiSelectSnapShot_Observer extends LoggableBase {
...
class UiManager_UiManager {
    ...
    InitUiManager() {
        ...
        this.ModuleSelectSnapShot.RegisterObserver(new UiManager_UiSelectSnapShot_Observer(this.Logger, this));
    ...
    SetUIStates(scUrlAgent, stateOfSitecoreWindow) {
    ...
    RefreshUiUIManagerFromStateOfSitecoreData(stateOfSitecoreWindow) {
        return UiManager_awaiter(this, void 0, void 0, function* () {
            this.Logger.FuncStart(this.RefreshUiUIManagerFromStateOfSitecoreData.name);
            yield this.SetUIStates(this.TabMan.GetScUrlAgent(), stateOfSitecoreWindow);
...
class CommandCompleted_Observer_CommandCompleted_Observer extends LoggableBase {
    constructor(logger, owner) {
        super(logger);
        this.Owner = owner;
    }
    ...
    InitHub() {
        return zPopUpEntryPoint_awaiter(this, void 0, void 0, function* () {
            try {
                ...
                let messageMan = new MessageManager_PopUpMessageManager(PopUpMessageBroker, this.Logger);
                ...
                let self = uiMan;
                handlers.External.RegisterObserver(new CommandCompleted_Observer_CommandCompleted_Observer(this.Logger, uiMan));
                uiMan.InitUiManager();
