...
class HandlersExternal_HandlersExternalEvent_Subject extends GenericEvent_Subject_GenericEvent_Subject {
    constructor(logger, msgManager, settingsAgent, tabMan) {
        super(logger, HandlersExternal_HandlersExternalEvent_Subject.name);
    ...
    SendCommandToContent(sendMsgPlayload) {
        return new Promise((resolve, reject) => HandlersExternal_awaiter(this, void 0, void 0, function* () {
            ...
            this.MessageManager.SendMessageToContentAsync(sendMsgPlayload)
                .then((replyMessagePayload) => this.NotifyObservers(replyMessagePayload))
                .then(() => resolve())
                .catch((err) => reject(err));
...
class Handlers_Handlers {
    constructor(logger, msgManager, settingsAgent, tabMan) {
        this.External = new HandlersExternal_HandlersExternalEvent_Subject(logger, msgManager, settingsAgent, tabMan);
...
class ButtonVisibilityTests_ButtonVisibilityTester extends LoggableBase {
    ...
    VisibilityTestActiveCeNode(stateOfSitecoreWindow) {
        let toReturn = false;
        if (stateOfSitecoreWindow) {
            let activeStateOfContentEditor = this.StateHelpers.GetActiveContentEditFromStateOfDesktop(stateOfSitecoreWindow.States.StateOfDesktop);
...
class SelectSnapshotModule_SelectSnapshotModule extends GenericEvent_Subject_GenericEvent_Subject {
    constructor(selector, logger) {
        super(logger, SelectSnapshotModule_SelectSnapshotModule.name);
        this.Selector = selector;
        this.StateHelpers = new StateHelpers_StateHelpers(this.Logger);
    }
    Init() {
...
class UiManager_UiManager {
    constructor(logger, settingsAgent, tabMan, commandMan) {
        ...
        this.ButtonStateManager = new UiButtonStateManager_UiButtonStateManager(this.Logger, this.CommandMan.AllMenuCommands);
    ...
    InitUiManager() {
        ...
        this.ModuleSnapShots.RegisterObserver(new UiSelectSnapShot_Observer_UiSelectSnapShot_Observer(this.Logger, this));
    ...
    InitHub() {
        return zPopUpEntryPoint_awaiter(this, void 0, void 0, function* () {
            try {
                ...
                let self = uiMan;
                handlers.External.RegisterObserver(new CommandCompleted_Observer_ContentReplyReceivedEvent_Observer(this.Logger, uiMan));
                uiMan.InitUiManager();
